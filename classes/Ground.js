class Ground {
    ...
    async init() {
        ...
        const material = new THREE.MeshStandardMaterial({color: 0x888888, side: THREE.DoubleSide, 
            map: ground, normalMap: normal, aoMap: ao, 
            displacementMap: null, displacementScale: 0.1, displacementBias: 0.01,
            roughnessMap: roughness, roughness: 0.6
        });


        this.mesh = await this.loadModel('river1', material);
        this.mesh.scale.x = 3;
        this.mesh.scale.y = 3;
        this.mesh.scale.z = 3;
